...
const FileUpload = () => {
  ...
  const handleSuccess = async (result) => {
    ...
    if (result.info && result.info.url) {
      setImageUrl(result.info.url);
      await saveImageUrl(result.info.url);
    }
  };
  ...
  return (
    <Container>
      <div>
        ...
        {isChecked && (
          <Form>
            <Label pointing='right'>Please Upload your Accreditation
Document</Label>
            <CldUploadButton
              style={{ marginLeft: "10px", padding: "10px",
backgroundColor: "teal", color: "white", border: 'none', borderRadius:
"3px" }}
              uploadPreset={process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET}
              onSuccess={handleSuccess}
              onError={handleError}
        ...
        {imageUrl && (
          <div>
            <h2>Uploaded Image:</h2>
            <img src={imageUrl} alt="Uploaded Content" style={{
maxWidth: '100%', marginTop: '20px' }} />
            <p>Image URL: <a href={imageUrl} target="_blank"
rel="noopener noreferrer">{imageUrl}</a></p>
          </div>
        )}
